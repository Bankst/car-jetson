...
- **Quit-key collision**: old script mapped lowercase `q` to "low band -1 dB" while `Q` was quit, so users pressing `q` to exit got an unwanted bump. New 8-band UI uses digits `1..8` to pick the active band and `-/+`/`[`/`]` for gain bumps; both `q` and `Q` quit.
- **Sluggish Ctrl-C**: previous `read -rsn1 -t 86400` swallowed signals for nearly a day. Now `read -rsn1 -t 1` for snappy SIGINT delivery without affecting interactive feel.
